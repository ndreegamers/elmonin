...
import { Loader2, Send, CheckCircle2, Ticket, ArrowUp } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
function StepBadge({ number, label, highlight }: { number: number; label: string; highlight?: boolean }) {
  return (
    <div className={cn(
      "flex items-center gap-2 mb-2",
      highlight ? "text-[#FACC15]" : "text-[#94A3B8]"
    )}>
      <span className={cn(
      ...
      <span className="text-xs font-orbitron uppercase tracking-wider">{label}</span>
    </div>
  );
}
    ...
    <Dialog open={open} onOpenChange={(o) => { if (!o && !submitting) onClose(); }}>
      <DialogContent className="bg-[#12121A] border-[#2A2A3E] max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="bg-[#1A1A2E] rounded-xl p-4 flex flex-col gap-2 border border-[#2A2A3E]">
            <div className="flex justify-between items-center">
              ...
              <span className="font-orbitron font-bold text-[#F1F5F9]">
                {ticketsPaid}
                {ticketsBonus > 0 && (
                  <span className="ml-1" style={{ color: "#FACC15" }}>+{ticketsBonus} gratis</span>
                )}
              </span>
            </div>
            {ticketsBonus > 0 && (
          ...
          {/* Step 1 — QR */}
          <div>
            <StepBadge number={1} label="Escanea y paga" />
            <div className="flex flex-col items-center gap-3">
              <div className="flex items-center justify-center gap-4">
                <div className="w-14 h-14 rounded-2xl overflow-hidden bg-white flex items-center justify-center p-1">
                  <Image
              ...
              <p className="text-xs font-orbitron text-[#94A3B8] uppercase tracking-widest text-center">
          ...
          {/* Step 2 — Receipt uploader */}
          <div>
            <StepBadge number={2} label="Adjunta tu comprobante" highlight={!receiptFile} />
            <ReceiptUploader
          ...
          {/* Step 3 — Submit */}
          <div>
            <StepBadge number={3} label="Enviar" highlight={!!receiptFile && !submitting} />
            <motion.button
